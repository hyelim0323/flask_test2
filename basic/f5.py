'''
    - POST 방식으로 데이터 전송하기
        - 클라이언트 (Json, Xml, Text, Form(키=값&키=값..), Form-encode, Graphql, Binary)
            - form 전송, 화면 껌뻑 => 화면 전환, => (Form, Form-encode 방식)
...
app = Flask(__name__)
# 세션을 위해서 시크릿키 지정
app.secret_key = 'abcdefghijklmnop' # 임의값, 통상 해시값 활용

# 로그인하여 세션을 얻은 후 홈페이지를 진행해야 사이트의 내용을 보여주겠다 -> 컨셉
@app.route('/')
def home():
    if not 'uid' in session:    # 세션안에 uid값이 존재하는가?
        # return redirect('/login')   # url을 사용할 때는 하드코딩을 하지 않는다.
...
def login():
    ...
    if request.method == 'GET':
        ...
        # 3. 회원이면
            ...
            #       클라이언트가 간편하게 웹을 이용할 수 있게 도움을 줌
            #       단점 : 접속유저가 많으면 서버측 메모리에 부하가 온다 -> 대체재 필요/대안필요
